...
class Style {
    ...
    public static function load() {
        ...
        add_action("wp_enqueue_scripts", array(__CLASS__, "enqueue_script_data"), 25);
        add_action("wp_enqueue_scripts", array(__CLASS__, "enqueue_scripts"), 20);

        /* enqueue scripts and stylesheets for pixelpad directly from pixelpad server */
        add_action("get_footer", array(__CLASS__, "pixelpad_enqueue_scripts"), 30);
        add_action("get_footer", array(__CLASS__, "pixelpad_enqueue_styles"), 30);

        // remove bloat
        add_action('wp_enqueue_scripts', array(__CLASS__, 'dequeue_scripts'), 15);
        add_action('wp_enqueue_scripts', array(__CLASS__, 'dequeue_styles'), 15);
    }
    ...
    public static function enqueue_script_data() {
        ...
        wp_script_add_data('fontawesome', array('integrity', 'crossorigin'), array('sha384-fnmOCqbTlWIlj8LyTjo7mOUStjsKC4pOpQbqyi7RrhN7udi9RwhKkMHpvLbHG9Sr', 'anonymous'));
    }
    ...
    public static function enqueue_scripts() {
        ...
        wp_enqueue_script(
            ...
            get_stylesheet_directory_uri() . "/includes/javascript/compressed/jquery-ui-1.12.1.min.js",
            array("jquery"),
            "1.0.1"
        );
        wp_enqueue_script(
    ...
    public static function enqueue_styles() {
        ...
        wp_enqueue_style(
            'fontawesome',
            get_stylesheet_directory_uri() . "/includes/css/compressed/all.min.css",
            array(),
            "5.90.1",
            "all"
        );
        wp_enqueue_style(
            'jqueryuicss',
            get_stylesheet_directory_uri() . "/includes/css/compressed/jquery-ui.min.css",
            array(),
            "5.90.1",
            "all"
        );
